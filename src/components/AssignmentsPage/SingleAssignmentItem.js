...
export default function SingleAssignmentItem({ data = {}, index, type }) {
  const { results, questions, assignments } = React.useContext(
    AssignmentsContext
  );
  const { user, users, classes, courses, formatAssignment } = React.useContext(
    UserContext
  );
  ...
  function checkDone(assignment) {
    let tempDone = false;
    let checkQuestions = questions.filter(
      (record) => record.quiz_id === assignment._id
    );
