...
export default function ResultsList({
  ...
  React.useEffect(() => {
    let currentAssignment = assignments.find((record) => record._id === quizId);
    if (!currentAssignment) currentAssignment = {};
    let tempTitle = currentAssignment.title;

    let tempQuestions = questions.filter((record) => record.quiz_id === quizId);
    ...
    for (let i = 0; i < tempQuestions.length; i++) {
      ...
      let questionResult = results.find(
        (record) =>
  ...
  return (
    <ResultsListWrapper>
      <div className="container col-lg-8 mx-auto">
        <h2 className="">
          {title} <span className="main-text"> /{totalMarks} marks</span>
        </h2>
        <hr></hr>
        {quizQuestions.map((record, index) => (
          <AnswerListItem
            key={record._id}
            index={index}
            question={record}
